...
*/
package com.ysoft.jdo.book.rental.servlet.opr;
...
import com.ysoft.jdo.book.rental.servlet.service.*;
...
public class ReservationServlet extends HttpServlet
   ...
   private volatile boolean    one_shot_debugging = true;
   ...
   private void respond(HttpServletRequest request, HttpServletResponse response)
      ...
      try
         ...
         // any exceptions from this point on will NOT
         // result in an exception page shown in the browser.
         // Instead, the exception is logged by GenericServlet.
         // make the persistent objects in the model transient
         Collection dataObjects = model.getDataObjects();
         service.makeTransientAll(dataObjects);

         // debugging
         if (one_shot_debugging)
            {
            TestSerializable.testSerialization(model, "the model");
            one_shot_debugging = false;
            }
         }
...
class TestSerializable
   {
   public static void testSerialization(Object o, String msg)
      {
      try
         {
